...
export default function KFCScreen() {
  ...
  const filteredMenu = kfcMenu.filter((item) =>
    item.name.toLowerCase().includes(search.toLowerCase())
  );

  const handleSkip = () => {
    router.push('/kfcgoole');
  };
  ...
  return (
    <ScrollView style={styles.container}>
      {/* Restaurant Info */}
      <Image source={require('./im/kfc.jpg')} style={styles.restaurantImage} />
      <Text style={styles.restaurantName}>KFC</Text>
      <Text style={styles.description}>Finger-licking good chicken!</Text>
      ...
      {selectedItem && (
        <View style={styles.formContainer}>
          <Text style={styles.formTitle}>
            Order: {selectedItem.name} (${selectedItem.price})
          </Text>
